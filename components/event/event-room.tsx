...
import TippingPanel from "./tipping-panel"
import ChatPanel from "./chat-panel"
import { fetchEvent, verifyTicket } from "../../services/event-service"
import { useToast } from "@/hooks/use-toast"
import { useWallet } from "@solana/wallet-adapter-react"
import { sendTip } from "../../services/tipping-service"
...
export default function EventRoom({ eventId }: EventRoomProps) {
    ...
    const loadEventAndVerifyTicket = async () => {
      try {
        ...
        if (wallet.connected && wallet.publicKey) {
          ...
          const ticketVerified = await verifyTicket(eventId, wallet.publicKey.toString())
  ...
  const handleTip = async (amount: number) => {
    ...
    try {
      ...
      const txHash = await sendTip(eventId, amount, wallet)

      toast({
        title: "Tip Sent",
        description: `You tipped ${amount} SOL to the artist. Transaction: ${txHash.slice(0, 8)}...`,
      })
  ...
  if (error || !event) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        ...
        {!wallet.connected && (
          <button className="mt-4 px-4 py-2 bg-primary text-white rounded-md" onClick={() => wallet.connect()}>
            Connect Wallet
          </button>
        )}
        {wallet.connected && !hasTicket && (
          <button
